...
ARGUMENTS = [
    DeclareLaunchArgument('control_config', default_value='control.yaml',
                          description='ros2_control config filename, forwarded '
                                      'to the diffdrive controller spawner'),
...
def generate_launch_description():
    ...
    motion_control_node = Node(
        ...
        parameters=[{
            'use_sim_time': True,
            # "full" rather than "backup_only". With safety active the Create 3
            # motion controller clamps linear velocity to 0.306 m/s, which
            # silently overrides whatever Nav2 asks for and is the real reason
            # the robot crawled. "full" lifts that to the diffdrive limit of
            # 0.46 m/s, so whatever top speed your Nav2 configuration asks for
            # actually takes effect. Safe here because this is simulation only.
